...
type Repository interface {
	FindAll(ctx context.Context, limit, offset int) ([]Actor, int64, error)
...
func (r *repository) FindAll(ctx context.Context, limit, offset int) ([]Actor, int64, error) {
	db := database.GetDB()
	if db == nil {
	...
	// Count total
	var total int64
	err := db.QueryRow(ctx, "SELECT COUNT(*) FROM actors").Scan(&total)
	if err != nil {
		return nil, 0, err
	}
